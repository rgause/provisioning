...
/**
 * REST controller for managing {@link com.mycompany.provisioning.domain.Tenant}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<Tenant> result = tenantRepository
            ...
            .map(
                existingTenant -> {
                    ...
                    if (tenant.getProvisioned() != null) {
                        existingTenant.setProvisioned(tenant.getProvisioned());
                    }
                    if (tenant.getFrpId() != null) {
                        existingTenant.setFrpId(tenant.getFrpId());
                    }
                    if (tenant.getFrpName() != null) {
                        existingTenant.setFrpName(tenant.getFrpName());
                    }
                    if (tenant.getFrpContractTypeCode() != null) {
                        existingTenant.setFrpContractTypeCode(tenant.getFrpContractTypeCode());
                    }
                    if (tenant.getFrpContractTypeIndicator() != null) {
                        existingTenant.setFrpContractTypeIndicator(tenant.getFrpContractTypeIndicator());
                    }
